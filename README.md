...
首页、画布和素材生成页统一支持 **5 / 10 / 30 秒**。5 / 10 秒可选 HappyHorse 或万相 3.0；30 秒仅匹配万相 3.0 原生生成，不拆段、不自动降为短视频。
新增万相 3.0 文生视频和图生视频目录项，共用百炼供应商 Key / Base URL。需账号已开通对应地域模型；图生视频按官方 `input.media` / `first_frame` 参数提交。
参数依据：[万相 3.0 官方 API](https://help.aliyun.com/zh/model-studio/wan3-video-generation-api-reference)。原供应商设置和已有模型价格不会被覆盖；管理员需为新增模型设置业务积分价，阿里云实际收费以官方账单为准。

- API 主机需要 Node.js 22.5+、`ffmpeg`（含 libx264 / AAC）和 `ffprobe`。Debian / Ubuntu 可运行 `apt-get install ffmpeg`；Docker API 镜像也需安装 FFmpeg。
- 新任务按所选模型单次积分价计费，提交时固定报价、生成成功后扣一次积分，不再乘分段数量。
...
- 首页和素材库每页 12 个素材，`GET /api/assets` 只返回轻量展示字段，不传完整模型响应。按图片/视频筛选及分页，沿用任务归属权限。
- 默认只显示可见卡片的压缩 JPG 封面；图片最长边 480 像素，保留比例。鼠标悬停或键盘聚焦后才播放 3 秒静音 MP4（12 fps、最长边 384 像素），全列表同时最多播放一条。
- 点击卡片后才加载原图或完整视频，下载仍是原文件。滚出可视区域、移开鼠标或切换后台标签页时卸载预览播放器；系统设置减少动态效果时只显示封面。
- `GET /api/jobs/:id/preview` 复用任务归属权限，按需为已有和新生成视频补预览。只处理已保存到 `data/outputs/` 的本地文件，不重新下载历史远程链接。不可生成时保留点击播放原视频的入口。
- `GET /api/jobs/:id/thumbnail?index=N` 为任务图片生成压缩封面，索引来自素材列表；同样校验归属，磁盘缓存使用 `任务ID__thumb-内容指纹` 命名。
